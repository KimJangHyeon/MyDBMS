...
void 
catalog_update(LeafPage* leaf, unumber_t num_col, unumber_t num_key, udata_t value[NMAXVAL]) {
	udata_t min, max;
	for (int i = 0; i < num_col - 1; i++) {
		if (num_key >= 1) {
			min = leaf->catalog.info[i].min;
			...
			printf("update num_key is 0 error!!!\n");
			exit(0);
		}
		if (min > value[i]) 
			leaf->catalog.info[i].min = value[i];
...
udata_t* 
do_find(utable_t tid, ukey64_t key) {
...
void
do_update(utable_t tid, ukey64_t key, udata_t value[], int* result) {
	unumber_t num_col;
	ukey32_t num_keys;

	LeafPage* leaf_page = (LeafPage*)malloc(sizeof(LeafPage));
	uoffset_t leaf_offset;
	num_col = get_col(tid);
	...
	num_keys = leaf_page->header_top.num_keys;
	catalog_update(leaf_page, num_col, num_keys, value);
	for (int i = 0; i < num_keys; i++) { 
		if (leaf_page->record[i].key == key) {
			memcpy(leaf_page->record[i].value, value, VALUESIZE);
			*result = UPDATE_SUCCESS;
			write_buffer(tid, leaf_offset, (Page*)leaf_page);
			return;
		}
	}
	*result = UPDATE_KEY_NOTFOUND;
}
void
do_insert(utable_t tid, ukey64_t key, udata_t value[]) {
...
void
do_erase(utable_t tid, ukey64_t key) {
	//ustring_t str = (char*)malloc(sizeof(char) * VALUESIZE);
	udata_t* temp;
	uoffset_t koffset;
